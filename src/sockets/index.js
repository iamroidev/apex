...
module.exports = function(io, db, logger) {
  io.on('connection', (socket) => {
    ...
    socket.on('rename-participant', ({ roomId, displayName }) => { if (roomId && state.rooms.has(roomId)) { const p = state.rooms.get(roomId).get(socket.id); if (p) { p.displayName = displayName; io.to(roomId).emit('participant-renamed', { socketId: socket.id, displayName }); } } });
    socket.on('speech-transcription', ({ roomId, text, final }) => { const room = state.rooms.get(roomId); const pInfo = room ? room.get(socket.id) : null; socket.to(roomId).emit('speech-transcription-broadcast', { senderId: socket.id, senderName: pInfo ? pInfo.displayName : 'Participant', text, final }); });
    ...
    socket.on('toggle-cohost', ({ roomId, targetSocketId }) => { if (!state.rooms.has(roomId)) return; const room = state.rooms.get(roomId); const sender = room.get(socket.id); if (!sender || sender.role !== 'host') return; const target = room.get(targetSocketId); if (!target) return; target.role = target.role === 'cohost' ? 'participant' : 'cohost'; io.to(roomId).emit('role-changed', { socketId: targetSocketId, role: target.role }); });
    socket.on('mute-participant', ({ roomId, targetSocketId }) => { if (!state.hasModPowers(roomId, socket.id)) return; io.to(targetSocketId).emit('mute-command'); });
    socket.on('mute-all', ({ roomId }) => { if (!state.hasModPowers(roomId, socket.id)) return; socket.to(roomId).emit('mute-command'); });
    socket.on('kick-participant', ({ roomId, targetSocketId }) => { if (!state.hasModPowers(roomId, socket.id)) return; io.to(targetSocketId).emit('kick-command'); });
    ...
    socket.on('stop-video-participant', ({ roomId, targetSocketId }) => { if (!state.hasModPowers(roomId, socket.id)) return; io.to(targetSocketId).emit('stop-video-command'); });
    socket.on('ask-unmute-participant', ({ roomId, targetSocketId }) => { if (!state.hasModPowers(roomId, socket.id)) return; io.to(targetSocketId).emit('unmute-request-prompt'); });

    // Security toggles
    socket.on('lock-room', ({ roomId, locked }) => { if (!state.hasModPowers(roomId, socket.id)) return; if (locked) state.lockedRooms.add(roomId); else state.lockedRooms.delete(roomId); io.to(roomId).emit('room-lock-changed', { locked }); });
    socket.on('toggle-waiting-room', ({ roomId, enabled }) => { if (!state.hasModPowers(roomId, socket.id)) return; if (enabled) state.waitingRooms.add(roomId); else state.waitingRooms.delete(roomId); io.to(roomId).emit('waiting-room-changed', { enabled }); });
    socket.on('spotlight-participants', ({ roomId, targetSocketIds }) => { if (!state.hasModPowers(roomId, socket.id)) return; state.spotlightQueue.set(roomId, targetSocketIds || []); io.to(roomId).emit('spotlight-updated', { spotlightSocketIds: targetSocketIds || [] }); });
    ...
    // Polling
    socket.on('poll-create', ({ roomId, poll }) => { if (!state.hasModPowers(roomId, socket.id)) return; socket.to(roomId).emit('poll-created', poll); });
    ...
    socket.on('poll-end', ({ roomId, pollId, results }) => { if (!state.hasModPowers(roomId, socket.id)) return; io.to(roomId).emit('poll-ended', { pollId, results }); });

    // Breakout rooms
    socket.on('create-breakout', ({ roomId, groups }) => { if (!state.hasModPowers(roomId, socket.id)) return; io.to(roomId).emit('breakout-created', { groups }); });
    ...
    socket.on('breakout-end', ({ roomId, roomCount }) => { if (!state.hasModPowers(roomId, socket.id)) return; for (let i = 1; i <= roomCount; i++) io.to(`${roomId}-breakout-${i}`).emit('breakout-ended'); });
    socket.on('breakout-broadcast-message', ({ roomId, message, roomCount }) => { if (!state.hasModPowers(roomId, socket.id)) return; io.to(roomId).emit('breakout-broadcast-received', { message }); for (let i = 1; i <= roomCount; i++) io.to(`${roomId}-breakout-${i}`).emit('breakout-broadcast-received', { message }); });
    ...
    socket.on('screenshare-grant-control', ({ roomId, targetSocketId }) => { if (!state.hasModPowers(roomId, socket.id)) return; state.screenShareControllers.set(roomId, { presenterSocketId: socket.id, controllerSocketId: targetSocketId }); io.to(targetSocketId).emit('screenshare-control-granted', { presenterSocketId: socket.id }); });
    socket.on('screenshare-revoke-control', ({ roomId }) => { if (!state.hasModPowers(roomId, socket.id)) return; const sc = state.screenShareControllers.get(roomId); if (sc) io.to(sc.controllerSocketId).emit('screenshare-control-revoked'); state.screenShareControllers.delete(roomId); });
    socket.on('video-filter-change', ({ roomId, filter }) => { socket.to(roomId).emit('video-filter-changed', { socketId: socket.id, filter }); });

    // Slides
    socket.on('slide-share-start', ({ roomId, slideIndex, slides }) => { if (!state.hasModPowers(roomId, socket.id)) return; io.to(roomId).emit('slide-share-started', { presenterSocketId: socket.id, slideIndex, slides }); });
    socket.on('slide-share-stop', ({ roomId }) => { if (!state.hasModPowers(roomId, socket.id)) return; state.slideControllers.delete(roomId); io.to(roomId).emit('slide-share-stopped'); });
    socket.on('slide-change', ({ roomId, slideIndex }) => { const isMod = state.hasModPowers(roomId, socket.id); const hasControl = state.slideControllers.get(roomId) === socket.id; if (!isMod && !hasControl) return; io.to(roomId).emit('slide-changed', { slideIndex }); });
    socket.on('slide-grant-control', ({ roomId, targetSocketId }) => { if (!state.hasModPowers(roomId, socket.id)) return; state.slideControllers.set(roomId, targetSocketId); io.to(roomId).emit('slide-control-granted', { targetSocketId }); });
    socket.on('slide-revoke-control', ({ roomId }) => { if (!state.hasModPowers(roomId, socket.id)) return; state.slideControllers.delete(roomId); io.to(roomId).emit('slide-control-revoked'); });

    const VALID_PERMISSIONS = ['none', 'host-only', 'public', 'public-private'];
    socket.on('change-chat-permissions', ({ roomId, permissions }) => { if (!state.hasModPowers(roomId, socket.id)) return; if (!VALID_PERMISSIONS.includes(permissions)) return; state.chatPermissions.set(roomId, permissions); io.to(roomId).emit('chat-permissions-changed', { permissions }); });
    ...
    socket.on('mute-all-except-presenter', ({ roomId, presenterSocketId }) => { if (!state.hasModPowers(roomId, socket.id)) return; socket.to(roomId).emit('mute-all-except-presenter-command', { presenterSocketId }); });
