...
module.exports = function(db, env) {
  ...
  router.post('/auth/logout', (req, res) => { clearSessionCookie(res, req); res.json({ ok: true }); });
  ...
  router.get('/sessions', authenticate, (req, res) => { res.json(db.listSessions(req.user.userId)); });
  ...
  router.get('/sessions/:id/attendance', authenticate, (req, res) => { 
    const limit = parseInt(req.query.limit) || 1000;
    const offset = parseInt(req.query.offset) || 0;
    res.json(db.getAttendance(req.params.id, limit, offset)); 
  });
  router.get('/sessions/:id/chat', authenticate, (req, res) => { 
    const limit = parseInt(req.query.limit) || 1000;
    const offset = parseInt(req.query.offset) || 0;
    res.json(db.getChatHistory(req.params.id, limit, offset)); 
  });

  router.get('/sessions/:id/export/:format', authenticate, (req, res) => {
  ...
  router.get('/scheduled', authenticate, (req, res) => { res.json(db.listScheduledMeetings(req.user.userId)); });
  router.post('/scheduled', authenticate, (req, res) => {
  ...
  router.delete('/scheduled/:id', authenticate, (req, res) => { db.deleteScheduledMeeting(req.params.id); res.json({ ok: true }); });
